...
export default function Categories() {
  ...
  const createMutation = useMutation(async () => {
    ...
    await createCategory({ name: newName.trim(), parent_id: newParent ? Number(newParent) : null })
    setNewName('')
    setNewParent('')
    await reload()
  })
  ...
  const saveMutation = useMutation(async () => {
    if (!editing) return
    await updateCategory(editing.id, { name: editing.name, parent_id: editing.parent_id })
    setEditing(null)
    await reload()
  })
  ...
  return (
    <div>
      ...
      <table>
        <thead><tr><th>Name</th><th>Parent</th><th></th></tr></thead>
        <tbody>
          {data.map(c => (
            <tr key={c.id}>
              <td>
